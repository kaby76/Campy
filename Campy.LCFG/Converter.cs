...
//
		...
		// .globl       nn_3
				.visible .func  (.param .b32 func_retval0) nn_4
				(
				 .param .b64 nn_4_param_0
				)
				;

		.visible .entry nn_3(
							 .param .u64 nn_3_param_0,
							 .param .u64 nn_3_param_1
							)
		{
			.reg .b32       %r<3>;
			.reg .b64       %rd<7>;

			ld.param.u64    %rd1, [nn_3_param_0];
			cvta.to.global.u64      %rd2, %rd1;
			ld.param.u64    %rd3, [nn_3_param_1];
...
//
						...
						)
	{
									...
									)
		{
			...
			ld.param.u64    %rd1, [nn_6_param_0];
			cvta.to.global.u64      %rd2, %rd1; // this
			ld.param.u64    %rd3, [nn_6_param_1];  // index
			ld.global.u32   %r1, [%rd2];   // step
			...
			ld.param.b32    %r2, [retval0+0]; // tid.x
			} // callseq 2

// test
        add.u64 %rd3, %rd2, 4;
 		   ld.global.u64   %rd4, [%rd3];  // closure
		  // ld.global.u32  %rd5, [%rd4];  // data
           st.u64 [%rd2], %rd4;
// test


			//mul.lo.s32      %r3, %r1, %r2;  //  tid.x * step  (= i)
			//ld.global.u64   %rd4, [%rd2+4];  // closure
			//ld.u64  %rd5, [%rd4];  // data
			//add.s64         %rd6, %rd5, 12; // (data + 12)

//			mul.wide.s32    %rd7, %r3, 4; // i * 4
//			add.s64         %rd8, %rd6, %rd7; // (data + 12) + i * 4
//			ld.u32  %r4, [%rd8]; // *((data + 12) + i * 4)
//			ld.global.u32   %r5, [%rd2];   // step
//
//			shr.u32         %r6, %r5, 31;  // step >> 31???
//			add.s32         %r7, %r5, %r6;  // (step >> 31) + step
//			shr.s32         %r8, %r7, 1;  // ((step >> 31) + step) >> 1
//			add.s32         %r9, %r3, %r8;
//			mul.wide.s32    %rd9, %r9, 4;
//			add.s64         %rd10, %rd6, %rd9;
//			ld.u32  %r10, [%rd10];
...
//			st.u32  [%rd6], 2;
        ...
        public unsafe CUfunction GetPtr(int block_number)
        {
            ...
            var the_name = Converter.NormalizeName(Converter.MethodName(lvv.Method));
        ...
        public static string NormalizeName(string before)
        {
            string result = "";
            ...
            Dictionary<TypeReference, System.Type> generic_type_rewrite_rules)
        {
            ...
            if (tr.IsArray)
            {
                ...
                TypeRef s = LLVM.StructCreateNamed(c, Converter.NormalizeName(tr.ToString()));
